...
**Известные сильные стороны:** лучший coding в реестре (SWE-bench 77.2%), vision, 201 язык, огромный контекст (262K). Единственная активная модель в реестре с vision (llama-4-scout deprecated Jul 17, 2026).
**Известные слабости:** thinking mode обязательно отключать (reasoning_effort="none"); dense архитектура — дороже на inference чем MoE при тех же параметрах; галлюцинации на изображениях без предупреждения (VQ-03 обязателен).
**Назначена на роли:** GENERAL (primary), VISION (primary), LONG_CONTEXT (primary), MULTILINGUAL (primary).
