...
_ADVICE_MARKERS = {
    ...
    "how can i go", "how do i go", "how to go", "how much does it cost",
    "what do i need", "what is needed", "visa", "budget for trip", "trip budget",
    "what should i know", "best place to visit", "best city to visit",
    "посовет", "что вкусного", "что попробовать", "куда поехать", "какой город лучше",
    "что поесть", "что посетить", "лучший город", "совет", "рекомендац",
    "сколько стоит", "что нужно для поездки", "что нужно", "виза", "бюджет",
    "как поехать", "как добраться", "что посмотреть", "что посмотреть в",
