...
async def handle_vision(
    ...
    # ── classify caption (user intent) only — never extracted content ─────────
    # Classifier works on USER INPUT only, not on LLM/vision output.
    # extracted = image description generated by LLM  → not a user request → never classify.
    # caption   = what the user actually typed/asked  → valid input for classifier.
    #
    # No caption → intent_result = None → orchestrator classifies via its own
    # fallback using extracted as user_message → description → CONVERSATION → describe.
    # This is the correct behaviour: no question = just describe the image.
...
async def handle_vision_group(
    ...
    # ── classify caption (user intent) only — never extracted content ─────────
    # Same rule as handle_vision(): classifier works on USER INPUT only.
    ...
    # No caption → intent_result = None → orchestrator fallback → CONVERSATION → describe.
